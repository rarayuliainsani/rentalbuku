Public Class WebFormPelanggan
    Inherits System.Web.UI.Page
    Sub load_gridview()
        If sql_conn(Session("currCon")) = True Then
            Dim tbl_grid As New DataTable
            tbl_grid = sql_datatable(Session("currCon"), "select * from pelanggan")
        ...
        Try
            If sql_conn(Session("currCon")) = True Then
                ...
                If txtstate.Text = "new" Then
                    sql = "insert into pelanggan values ('{0}','{1}','{2}','{3}')"
                    MsgBox("Data berhasil ditambahkan", MsgBoxStyle.Information)
                    sql = String.Format(sql, txtId.Text, txtNama.Text, txtAlamat.Text, txtNotelepon.Text)
                    sql_execute(Session("currCon"), sql)

                ElseIf txtstate.Text = "edit" Then

                    sql = "update pelanggan set nama='{0}',alamat='{1}',no_telp='{2}' where id='{3}'"
                    sql = String.Format(sql, txtNama.Text, txtAlamat.Text, txtNotelepon.Text, txtId.Text)
                    sql_execute(Session("currCon"), sql)
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        If sql_conn(Session("currCon")) = True Then
            sql_execute(Session("currCon"), "delete from pelanggan where id='" & txtId.Text & "'")
            sql_conn_close(Session("currCon"))
            load_gridview()
            object_clear()
        End If
